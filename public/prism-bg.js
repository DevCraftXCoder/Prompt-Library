// Prism animated background — vanilla JS port of the React Bits <Prism /> component.
// Uses ogl (loaded as ESM from esm.sh). Mounts into a host element (defaults to #prism-bg).
// Same shader, same uniforms, same prop semantics as the React version.
...
export function mountPrism(target, opts = {}) {
...
// Auto-mount: looks for #prism-bg and reads data-* attributes for overrides.
function autoMount() {
  const el = document.getElementById('prism-bg');
  ...
  mountPrism(el, {
    animationType: el.dataset.animationType || 'rotate',
    timeScale: num('timeScale', 0.5),
    height: num('height', 3.5),
    baseWidth: num('baseWidth', 5.5),
    scale: num('scale', 3.6),
    hueShift: num('hueShift', 0),
    colorFrequency: num('colorFrequency', 1),
    noise: num('noise', 0.5),
    glow: num('glow', 1),
    bloom: num('bloom', 1),
    suspendWhenOffscreen: bool('suspendWhenOffscreen', true),
